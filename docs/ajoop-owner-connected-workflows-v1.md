...
- **Sender:** a Turkish ablative (`Zaigo'dan`), `from <Name>`, or `Did/Has <Name> reply`. The token must match `^[\p{L}\p{N}][\p{L}\p{N}&._-]{0,59}$`, so Gmail operators, spaces, quotes and parentheses cannot enter the fixed query. For a coordinated expression (`ve`, `veya`, `ya da`, `ile`, `and`, `or`), the parser must consume the complete sender region and validate every segment under the same strict token rules; malformed, empty or more than eight segments fail closed with `ambiguous-sender` and zero reads. More than one distinct valid sender also clarifies, while repeated identical safe senders are deduplicated.
