...
import { AJOOP_DRIVE_READONLY_SCOPE, loadStoredDriveReadAuth } from "../server/drive-provider-client.mjs";

const DEFAULT_AUTH_TIMEOUT_MS = 5 * 60 * 1000;
const runtimeDirectory = path.resolve(".ajoop-runtime", "drive");
const credentialsPath = path.resolve(process.env.AJOOP_DRIVE_OAUTH_CLIENT_PATH || path.join(runtimeDirectory, "oauth-client.json"));
const tokenPath = path.resolve(process.env.AJOOP_DRIVE_TOKEN_PATH || path.join(runtimeDirectory, "oauth-token.json"));

export const readDesktopDriveClientCredentials = async (filePath) => {
...
export async function authorizeDesktopDriveRead({
  clientId,
  clientSecret,
  ...
  try {
    ...
    const authorizationUrl = oauthClient.generateAuthUrl({
      access_type: "offline",
      prompt: "consent",
      scope: [AJOOP_DRIVE_READONLY_SCOPE],
      state,
...
export async function runDriveAuthBootstrap({
  clientFilePath = credentialsPath,
  tokenFilePath = tokenPath,
  authorize = authorizeDesktopDriveRead,
} = {}) {
  ...
  if (tokenFileExists) {
    try {
      await loadStoredDriveReadAuth({ tokenPath: tokenFilePath });
  ...
  const client = await readDesktopDriveClientCredentials(clientFilePath);
...
const runCli = async () => {
  try {
    const result = await runDriveAuthBootstrap();
    const message = result.status === "exists" ? "already exists" : "stored locally";
    console.log(`Drive read-only authorization ${message} at ${result.tokenFilePath}`);
  } catch (error) {
    ...
    ].includes(code) ? code : "drive-auth-bootstrap-failed";
    console.error(`Drive authorization failed: ${safe}`);
    process.exitCode = 1;
  }
};
