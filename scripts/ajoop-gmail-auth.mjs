...
export const readDesktopClientCredentials = async (filePath) => {
...
export async function authorizeDesktopGmailRead({
  clientId,
  clientSecret,
  ...
  const server = createServer((request, response) => {
    try {
      ...
      if (requestUrl.searchParams.get("state") !== state) {
        ...
        settleCallback.reject(new Error("oauth-state-mismatch"));
        return;
      }
      ...
      if (providerError || !code) {
        ...
        settleCallback.reject(new Error("oauth-authorization-rejected"));
        return;
      }
      ...
      settleCallback.reject(new Error("invalid-oauth-callback"));
    }
  });
  ...
  try {
    ...
    timeout = setTimeout(() => settleCallback.reject(new Error("oauth-authorization-timeout")), timeoutMs);
...
export async function runGmailAuthBootstrap({
  clientFilePath = credentialsPath,
  tokenFilePath = tokenPath,
  authorize = authorizeDesktopGmailRead,
} = {}) {
  ...
  if (tokenFileExists) {
    try {
      await loadStoredGmailReadAuth({ tokenPath: tokenFilePath });
  ...
  const client = await readDesktopClientCredentials(clientFilePath);
...
const runCli = async () => {
  try {
    const result = await runGmailAuthBootstrap();
    const message = result.status === "exists" ? "already exists" : "stored locally";
    console.log(`Gmail read-only authorization ${message} at ${result.tokenFilePath}`);
  } catch (error) {
